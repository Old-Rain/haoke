...
class List extends React.Component {
  ...
  changeCity = ({ label, value }) => {
    if (!hasHouseInfoCity.includes(label))
      return Toast.info('暂无该城市的房源信息', 2)
  ...
  onPullUpBottom(e) {
    ...
    let totalHeight = this.getElementsByClassName(
        ...
        <AutoSizer>
          {({ width, height }) => (
            <VList
              ...
              onRowsRendered={this.onRowsRendered} // 滚动时返回4列表项位置的个索引
        ...
        <ul>
          {cityIndex.map((x, y) => {
            return (
              ...
              >
                <span className={startIndex === y ? 'active' : ''}>
                  {x === 'hot' ? '热' : x.toUpperCase()}
                </span>
              </li>
            )
          })}
    ...
    return (
      <div className="citylist">
        ...
        <CityIndex
          {...this.props}
          {...this.state}
          vListComponent={this.vListComponent}
        />
      </div>
    )
  }
